...
var Sugerencias = require('./app/models/sugerencias');
var Notas = require('./app/models/notas');
// configuration
...
apiRoutes.get('/sugerencias', function(req,res) {
	Sugerencias.find({}, function(error, sugerencias){
...
apiRoutes.post('/sugerencias', function(req, res){
	...
	if(author == undefined || titulo == undefined || desc== undefined){
		res.json(responseFormater(false, {}, "Author, titulo and description are required"));
	}
	else{
		Sugerencias.create({author : author, titulo : titulo, desc : desc}, function(error, sugerencias){
			if(error){
				console.log(error);
				console.trace();
...
apiRoutes.get('/notas', function(req, res){
	Notas.find({}, function(error, notas){
		res.json(JSON.stringify(notas));
	});
});

apiRoutes.post('/notas', function(req, res){
	var name = req.body.name;
	var text = req.body.text;

	if(name == undefined || text == undefined){
		res.json(responseFormater(false, {}, "Title and desc. are necessary"));
	}
	else{
		Notas.create({ name : name, text : text}, function(error, notas){
			if(error){
				console.log(error);
				console.trace();
				res.json(responseFormater(false, {}, "Error"));
				...
				res.json(responseFormater(true, { notas : notas}));
			}
		});
	}
});
